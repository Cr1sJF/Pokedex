...
import { apiResponse, pokemonApiResponse, pokemonServiceResponse, specieApiResponse, variety } from 'src/common/interfaces';
...
export class PokeApiService implements OnInit {
	...
	getPokemonIds(param: string) {
		return this.allSpecies.filter((specie: specieApiResponse) => {
		...
		return new Promise(async resolve => {
			if (this.speciesCache[param]) {
				...
				if (specieResponse.success) {
					...
					let specieApi: specieApiResponse = (specieResponse.res as specieApiResponse);
					let specie = new Specie(specieApi.name, specieApi.id, specieApi.flavor_text_entries);
					...
					while(specieApi.varieties[index]){
						...
						specie.addVariety(new MicroPokemon(pokeApi.name, pokeApi.id, pokeApi.sprites, pokeApi.types));
						index++;
					}
